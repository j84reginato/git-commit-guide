...
let conventionalCommitTypes = require('./types');
let defaults = require('./defaults');

let configLoader = require('commitizen').configLoader;
...
const options = {
  ...
  minHeaderWidth:
    (process.env.CZ_MIN_HEADER_WIDTH && parseInt(process.env.CZ_MIN_HEADER_WIDTH)) ||
...
(function(options) {
  try {
    let commitlintLoad = require('@commitlint/load');
    commitlintLoad().then(function(clConfig) {
      if (clConfig.rules) {
        let maxHeaderLengthRule = clConfig.rules['header-max-length'];
        if (
